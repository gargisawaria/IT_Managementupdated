...
namespace ITManagement.Controllers
{
    public class ManageUsersController : Controller
    {
        ...
        {
            List<Users> users = ITM.Users.ToList();
            List<Devices> devices = ITM.Devices.ToList();
            List<Categories> categories = ITM.Categories.ToList();

            Dictionary<int, string> categoryNames = categories.ToDictionary(c => c.CategoryId, c => c.CategoryName);

            foreach (var user in users)
            {
                var userDevices = devices.Where(d => d.UserId == user.UserId).ToList();
        ...
        public IActionResult UsersFilter(string selectedUsername)
        {
            var usersWithDevices = ITM.Users
                        .Where(u => (string.IsNullOrEmpty(selectedUsername) || u.FullName.Contains(selectedUsername, StringComparison.OrdinalIgnoreCase)) && !u.Isarchived)
                ...
                .Select(user => new Users
                {
                    ...
                    Email = user.Email,
                    Devices = ITM.Devices.Where(d => d.UserId == user.UserId).ToList()
                })
                .ToList();

            List<Categories> categories = ITM.Categories
